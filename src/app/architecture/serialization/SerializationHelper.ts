...
export class SerializationHelper {
	...
	static getJsonProperty<T>(target: any, propertyKey: string): IJsonMetaData<T> {
		...
		return Reflect.getMetadata(SerializationHelper.jsonMetadataKey, target, propertyKey);
	}
					...
					if (metadata.clazz || SerializationHelper.isPrimitive(clazz)) {
						...
						if (innerJson && SerializationHelper.isArray(innerJson)) {
							...
							return innerJson.map(
								(item) => SerializationHelper.deserialize(metadata.clazz, item));
						} else {
							return undefined;
						}
					} else {
	...
	public static Serialize(obj): string {
		...
		if (type != "object" || obj === null) {
			...
			for (n in obj) {
				...
				} else if (type == "object" && v !== null && check) v = SerializationHelper.Serialize(v);
